...
if [ "$DEBUG_LOGGING" = "enabled" ]; then
    # Enable all error reporting
    sed -i "s@^error_reporting = .*@error_reporting = E_ALL@" /etc/php5/*/php.ini

    # Set FPM Debug level
    sed -i "s@^;log_level = notice@log_level = debug@" /etc/php5/fpm/php-fpm.conf
fi

if [ "$APP_FOLDER" = "ezpublish" ]; then
    mv /etc/php5/mods-available/xdebug.ini-ezp54 /etc/php5/mods-available/xdebug.ini
fi
