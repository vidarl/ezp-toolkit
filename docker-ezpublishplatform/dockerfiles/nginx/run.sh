...
# Make sure nginx forwards to php-fpm container on tcp port, not unix socket
PHPFPM_HOSTNAME=${PHPFPM:-php_fpm}
sed -i "s@  fastcgi_pass unix:/var/run/php5-fpm.sock;@  # fastcgi_pass unix:/var/run/php5-fpm.sock;@" /etc/nginx/conf.d/ez.conf
sed -i "s@  #fastcgi_pass 127.0.0.1:9000;@  fastcgi_pass ${PHPFPM_HOSTNAME}:9000;@" /etc/nginx/conf.d/ez.conf
